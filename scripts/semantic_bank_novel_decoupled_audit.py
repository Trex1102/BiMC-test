...
def create_output_dir(cfg, train_cfg):
    ...
    trainer_name = f"semantic_bank_novel_decoupled_audit_{Path(train_cfg).stem}"
...
def build_variant_bundle(
    ...
    for cls_id in range(num_cls):
        ...
        if cls_id < num_base_cls or variant == "original":
            selected = desc_group
        else:
            if variant == "novel_random_fused_k":
                selected_idx = select_random_indices(desc_group, k, seed + cls_id * 17)
            elif variant == "novel_diverse_fused_k":
                selected_idx = select_diverse_indices(desc_group, k)
            elif variant == "novel_confuser_fused_k":
        ...
        fused_groups.append(selected)
        if cls_id < num_base_cls:
            fused_proto.append(original_description_proto[cls_id])
        else:
            fused_proto.append(normalize_vector(selected.mean(dim=0)))
    ...
    stats = {
        ...
        "novel_fused_count_mean": round(float(sum(novel_counts) / max(1, len(novel_counts))), 3) if novel_counts else 0.0,
        "knn_description_count_mean": round(float(full_description_features.shape[0] / num_cls), 3),
        "novel_within_class_diversity_mean": round(float(sum(novel_diversity) / max(1, len(novel_diversity))), 6) if novel_diversity else 0.0,
        "novel_support_alignment_mean": round(float(sum(novel_align_mean) / max(1, len(novel_align_mean))), 6) if novel_align_mean else 0.0,
        "novel_support_alignment_spread_mean": round(float(sum(novel_align_spread) / max(1, len(novel_align_spread))), 6) if novel_align_spread else 0.0,
        "novel_confuser_overlap_rate_mean": round(float(sum(novel_overlap) / max(1, len(novel_overlap))), 6) if novel_overlap else 0.0,
        "novel_confuser_similarity_mean": round(float(sum(novel_conf_sims) / max(1, len(novel_conf_sims))), 6) if novel_conf_sims else 0.0,
    }
    ...
    return {
        ...
        "fused_description_proto": torch.stack(fused_proto, dim=0),
...
def build_candidate_banks(bundle, num_cls, num_base_cls, lambda_t, base_atom_topk):
    text_features = F.normalize(bundle["text_features"][:num_cls], dim=-1)
    fused_description_proto = F.normalize(bundle["fused_description_proto"][:num_cls], dim=-1)
...
def evaluate_baseline(bundle, query_features, query_targets, num_cls, num_base_cls, beta, lambda_t, ensemble_alpha):
    ...
    semantic_proto = F.normalize((1.0 - lambda_t) * text_features + lambda_t * fused_description_proto, dim=-1)
...
def run_oracle(bundle, query_features, query_targets, num_cls, num_base_cls, beta, lambda_t, ensemble_alpha, base_atom_topk, max_passes):
    image_proto = F.normalize(bundle["image_proto"][:num_cls], dim=-1)
    ...
    baseline_sem, banks = build_candidate_banks(bundle, num_cls, num_base_cls, lambda_t, base_atom_topk)
...
def evaluate_variant(bundle, query_features, query_targets, num_cls, num_base_cls, beta, lambda_t, ensemble_alpha, base_atom_topk):
    baseline = evaluate_baseline(bundle, query_features, query_targets, num_cls, num_base_cls, beta, lambda_t, ensemble_alpha)
    oracle_full = run_oracle(bundle, query_features, query_targets, num_cls, num_base_cls, beta, lambda_t, ensemble_alpha, base_atom_topk, max_passes=2)
    oracle_fused = run_oracle(bundle, query_features, query_targets, num_cls, num_base_cls, beta, lambda_t, 1.0, base_atom_topk, max_passes=2)
    return {
...
def run_dataset(data_cfg, train_cfg, k, confuser_topk, base_atom_topk, seed_override=None):
    ...
    results = {}
    for variant_name in ("original", "novel_random_fused_k", "novel_diverse_fused_k", "novel_confuser_fused_k"):
    ...
    for variant_name, variant_payload in results.items():
        ...
        lines.extend([
            ...
            f"[{variant_name}] base_fused_count={s['base_fused_count_mean']} novel_fused_count={s['novel_fused_count_mean']} knn_count={s['knn_description_count_mean']} novel_diversity={s['novel_within_class_diversity_mean']} novel_confuser_overlap={s['novel_confuser_overlap_rate_mean']}",
        ])
    ...
    print(json.dumps({
        ...
        "results": {
            key: {
                ...
                "full_novel_acc": value["metrics"]["full_novel_acc"],
    ...
    print(f"Saved semantic bank novel-decoupled audit to: {out_dir}")
