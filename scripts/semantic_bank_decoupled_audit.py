...
def create_output_dir(cfg, train_cfg):
    ...
    trainer_name = f"semantic_bank_decoupled_audit_{Path(train_cfg).stem}"
...
def build_compact_variant(merged_state, variant, k, seed, reference_fused_proto, ref_confuser_sets, ref_confuser_proto):
    ...
    for cls_id in range(num_cls):
        ...
        elif variant == "random_fused_k":
            selected_idx = select_random_indices(desc_group, k, seed + cls_id * 17)
        elif variant == "diverse_fused_k":
            selected_idx = select_diverse_indices(desc_group, k)
        elif variant == "confuser_fused_k":
            selected_idx = select_confuser_aware_indices(desc_group, image_proto[cls_id], ref_confuser_proto[cls_id], k)
        else:
            raise ValueError(f"unknown variant: {variant}")

        selected = F.normalize(desc_group[selected_idx], dim=-1)
        compact_groups.append(selected)
        compact_proto.append(normalize_vector(selected.mean(dim=0)))
    ...
    stats = {
        "fused_description_count_mean": round(float(sum(counts) / max(1, len(counts))), 3),
        "fused_description_count_min": int(min(counts)),
        "fused_description_count_max": int(max(counts)),
        "knn_description_count_mean": round(float(full_description_features.shape[0] / num_cls), 3),
        "within_class_diversity_mean": round(float(sum(diversity_vals) / max(1, len(diversity_vals))), 6),
        "support_alignment_mean": round(float(sum(align_means) / max(1, len(align_means))), 6),
        "support_alignment_spread_mean": round(float(sum(align_spreads) / max(1, len(align_spreads))), 6),
        "confuser_overlap_rate_mean": round(float(sum(overlap_vals) / max(1, len(overlap_vals))), 6),
        "confuser_similarity_mean": round(float(sum(conf_sims) / max(1, len(conf_sims))), 6),
    }
    ...
    bundle = {
        ...
        "compact_proto": torch.stack(compact_proto, dim=0),
...
def build_decoupled_candidate_banks(bundle, num_cls, num_base_cls, lambda_t, base_atom_topk):
    text_features = F.normalize(bundle["text_features"][:num_cls], dim=-1)
    compact_proto = F.normalize(bundle["compact_proto"][:num_cls], dim=-1)
    baseline_sem = F.normalize((1.0 - lambda_t) * text_features + lambda_t * compact_proto, dim=-1)

    banks = []
    for cls_id in range(num_cls):
        atoms = [
            {"name": "prompt", "vector": text_features[cls_id], "uses_base": False, "uses_desc_atom": False},
            {"name": "desc_mean", "vector": compact_proto[cls_id], "uses_base": False, "uses_desc_atom": False},
        ]
        for atom_idx, atom in enumerate(bundle["compact_groups"][cls_id]):
            atoms.append({
        ...
        for atom in atoms[2:]:
            ...
            candidates.append({
                ...
                "vector": average_vectors([compact_proto[cls_id], atom["vector"]]),
                "uses_base": atom["uses_base"],
                "uses_desc_atom": atom["uses_desc_atom"],
                "is_baseline": False,
            })
        ...
        if desc_atom_vectors:
            candidates.append({
                ...
                "vector": average_vectors([text_features[cls_id], compact_proto[cls_id], desc_atom_vectors[0]]),
                "uses_base": False,
                "uses_desc_atom": True,
                "is_baseline": False,
            })
        ...
        for atom in [atom for atom in atoms if atom["uses_base"]]:
            candidates.append({
                ...
                "vector": average_vectors([text_features[cls_id], compact_proto[cls_id], atom["vector"]]),
                "uses_base": True,
                "uses_desc_atom": False,
                "is_baseline": False,
            })
...
def evaluate_decoupled_baseline(bundle, query_features, query_targets, num_cls, num_base_cls, beta, lambda_t, ensemble_alpha):
    image_proto = F.normalize(bundle["image_proto"][:num_cls], dim=-1)
    text_features = F.normalize(bundle["text_features"][:num_cls], dim=-1)
    compact_proto = F.normalize(bundle["compact_proto"][:num_cls], dim=-1)
    knn_desc_features = F.normalize(bundle["knn_description_features"], dim=-1)
    knn_desc_targets = bundle["knn_description_targets"]
    cov_image = bundle["cov_image"]

    semantic_proto = F.normalize((1.0 - lambda_t) * text_features + lambda_t * compact_proto, dim=-1)
    prob_cov = F.softmax(compute_cov_logits(query_features, image_proto, cov_image) / 512.0, dim=-1)
    prob_knn = F.softmax(compute_knn_logits(query_features, knn_desc_features, knn_desc_targets, num_cls), dim=-1)
...
def run_decoupled_oracle(bundle, query_features, query_targets, num_cls, num_base_cls, beta, lambda_t, ensemble_alpha, base_atom_topk, max_passes):
    image_proto = F.normalize(bundle["image_proto"][:num_cls], dim=-1)
    ...
    baseline_sem, banks = build_decoupled_candidate_banks(bundle, num_cls, num_base_cls, lambda_t, base_atom_topk)
    ...
    chosen_meta = {
        cls_id: {"name": "baseline_sem", "is_baseline": True}
        for cls_id in range(num_cls)
    }
        ...
        for cls_id in novel_classes:
            ...
            for candidate in banks[cls_id]:
                ...
                if acc > best_acc + 1e-6:
                    ...
                    best_meta = {"name": candidate["name"], "is_baseline": bool(candidate["is_baseline"])}
    ...
    changed = [cls_id for cls_id in novel_classes if not chosen_meta[cls_id]["is_baseline"]]
    return {
...
def evaluate_variant(bundle, query_features, query_targets, num_cls, num_base_cls, beta, lambda_t, ensemble_alpha, base_atom_topk):
    baseline = evaluate_decoupled_baseline(bundle, query_features, query_targets, num_cls, num_base_cls, beta, lambda_t, ensemble_alpha)
    oracle_full = run_decoupled_oracle(bundle, query_features, query_targets, num_cls, num_base_cls, beta, lambda_t, ensemble_alpha, base_atom_topk, max_passes=2)
    oracle_fused = run_decoupled_oracle(bundle, query_features, query_targets, num_cls, num_base_cls, beta, lambda_t, 1.0, base_atom_topk, max_passes=2)
    return {
        "full_acc": baseline["full_acc"],
        "full_base_acc": baseline["full_base_acc"],
        "full_novel_acc": baseline["full_novel_acc"],
        "fused_only_acc": baseline["fused_only_acc"],
        "fused_only_base_acc": baseline["fused_only_base_acc"],
        "fused_only_novel_acc": baseline["fused_only_novel_acc"],
        "knn_only_acc": baseline["knn_only_acc"],
        "knn_only_base_acc": baseline["knn_only_base_acc"],
...
def run_dataset(data_cfg, train_cfg, k, confuser_topk, base_atom_topk, seed_override=None):
    ...
    results = {}
    for variant_name in ("original", "random_fused_k", "diverse_fused_k", "confuser_fused_k"):
        bundle, stats = build_compact_variant(
            merged_state,
            variant_name,
            k_eff,
            int(cfg.SEED),
    ...
    for variant_name, variant_payload in results.items():
        ...
        lines.extend([
            ...
            f"[{variant_name}] fused_count_mean={s['fused_description_count_mean']} knn_count_mean={s['knn_description_count_mean']} diversity_mean={s['within_class_diversity_mean']} confuser_overlap={s['confuser_overlap_rate_mean']}",
        ])
    ...
    print(f"Saved semantic bank decoupled audit to: {out_dir}")
