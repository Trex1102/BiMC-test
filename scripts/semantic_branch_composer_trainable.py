...
from semantic_branch_composer_killswitch import optimize_class_oracle
...
def predict_episode_semantics(
    ...
    for cls_id in pseudo_novel_local:
        ...
        composer_vec = torch.sum(weights.unsqueeze(-1) * vectors, dim=0)
        composer_vec = normalize_vector(composer_vec)
        alpha = torch.clamp(outputs["alpha"].squeeze(0) * float(alpha_scale), min=0.0, max=1.0)
        semantic_vec = normalize_vector((1.0 - alpha) * baseline_sem[cls_id] + alpha * composer_vec)
        semantic_proto[cls_id] = semantic_vec
        semantic_by_class[int(cls_id)] = semantic_vec
        alphas.append(alpha)
        alpha_by_class[int(cls_id)] = alpha
    ...
    return semantic_proto, {
        ...
        "semantic_by_class": semantic_by_class,
        "alpha_by_class": alpha_by_class,
    }


def precompute_episode_oracle_targets(cache, cfg, max_desc_atoms, base_atom_topk, max_steps, lr):
    ...
    baseline_sem = F.normalize(
        (1.0 - lambda_t) * cache["state"]["text_features"][:cache["num_cls"]]
        + lambda_t * cache["state"]["description_proto"][:cache["num_cls"]],
        dim=-1,
    )

    oracle_targets = {}
    for cls_id in cache["pseudo_novel_local"]:
        _, component_bank, _, _ = component_bank_for_class(
            state=cache["state"],
            cls_id=cls_id,
        ...
        best = optimize_class_oracle(
            ...
            current_semantic=baseline_sem,
            component_bank=component_bank,
            image_proto=cache["state"]["image_proto"][:cache["num_cls"]],
            num_base_cls=cache["num_base_cls"],
            beta=beta,
            ensemble_alpha=ensemble_alpha,
            prob_cov=cache["prob_cov"],
            prob_knn=cache["prob_knn"],
            query_features=cache["query_features"],
            query_targets=cache["query_targets"],
            max_steps=max_steps,
            lr=lr,
            alpha_reg=0.0,
            entropy_reg=0.0,
        ...
        if best is None:
            oracle_targets[int(cls_id)] = {
                "semantic": baseline_sem[cls_id].detach().clone(),
                "alpha": torch.tensor(0.0, device=baseline_sem.device),
            }
        else:
            oracle_targets[int(cls_id)] = {
                "semantic": best["semantic_vec"].detach().clone(),
                "alpha": torch.tensor(float(best["alpha"]), device=baseline_sem.device),
            }
    return oracle_targets


def precompute_episode_cache(episode, cfg, max_desc_atoms, base_atom_topk, oracle_steps, oracle_lr):
    ...
    cache["oracle_targets"] = precompute_episode_oracle_targets(
        cache=cache,
        cfg=cfg,
        max_desc_atoms=max_desc_atoms,
        base_atom_topk=base_atom_topk,
...
def evaluate_episode(
    ...
    return {
        ...
        "semantic_by_class": composer_info["semantic_by_class"],
        "alpha_by_class": composer_info["alpha_by_class"],
    }
...
def train_composer(
    ...
    for epoch in range(max_epochs):
        ...
        for cache in train_caches:
            ...
            oracle_targets = cache.get("oracle_targets", {})
            if oracle_targets:
                semantic_losses = []
                alpha_losses = []
                for cls_id, target in oracle_targets.items():
                    pred_sem = metrics["semantic_by_class"][int(cls_id)]
                    pred_alpha = metrics["alpha_by_class"][int(cls_id)]
                    target_sem = target["semantic"]
                    target_alpha = target["alpha"]
                    semantic_losses.append(1.0 - torch.sum(pred_sem * target_sem))
                    alpha_losses.append((pred_alpha - target_alpha) ** 2)
                if semantic_losses:
                    loss = loss + float(distill_weight) * torch.stack(semantic_losses).mean()
                if alpha_losses:
                    loss = loss + float(alpha_target_weight) * torch.stack(alpha_losses).mean()
            loss.backward()
...
def select_alpha_scale(composer, val_caches, cfg, max_desc_atoms, base_atom_topk, alpha_scales):
    composer.eval()
    best_scale = float(alpha_scales[0])
    best_full = -1.0
    best_novel = -1.0
    summary = []
    with torch.no_grad():
        for alpha_scale in alpha_scales:
            ...
            for cache in val_caches:
                ...
                full_values.append(float(metrics["full_acc"].item()))
                novel_values.append(float(metrics["novel_acc"].item()))
            mean_full = float(np.mean(full_values) * 100.0 if full_values else 0.0)
            mean_novel = float(np.mean(novel_values) * 100.0 if novel_values else 0.0)
            summary.append({
                "alpha_scale": float(alpha_scale),
                "val_full": round(mean_full, 3),
                "val_novel": round(mean_novel, 3),
            })
            if mean_full > best_full + 1e-6 or (abs(mean_full - best_full) <= 1e-6 and mean_novel > best_novel + 1e-6):
                best_scale = float(alpha_scale)
                best_full = mean_full
                best_novel = mean_novel
    return {
        "best_alpha_scale": round(best_scale, 3),
        "best_alpha_val_full": round(best_full, 3),
        "best_alpha_val_novel": round(best_novel, 3),
        "alpha_scale_summary": summary,
    }
...
def run_dataset(
    ...
    train_caches = [precompute_episode_cache(ep, cfg, max_desc_atoms, base_atom_topk, oracle_steps, oracle_lr) for ep in train_episodes]
    val_caches = [precompute_episode_cache(ep, cfg, max_desc_atoms, base_atom_topk, oracle_steps, oracle_lr) for ep in val_episodes]
    ...
    train_summary = train_composer(
        ...
        alpha_target_weight=alpha_target_weight,
    )
    alpha_summary = select_alpha_scale(
        composer=composer,
        val_caches=val_caches,
        cfg=cfg,
        max_desc_atoms=max_desc_atoms,
    ...
    final_results = evaluate_final_session(
        ...
        max_desc_atoms=max_desc_atoms,
        base_atom_topk=base_atom_topk,
        alpha_scale=alpha_summary["best_alpha_scale"],
    )
    ...
    payload = {
        ...
        "alpha_target_weight": float(alpha_target_weight),
        "oracle_steps": int(oracle_steps),
        "oracle_lr": float(oracle_lr),
        "alpha_scale_candidates": list(map(float, alpha_scales)),
        "best_alpha_scale": alpha_summary["best_alpha_scale"],
        "best_alpha_val_full": alpha_summary["best_alpha_val_full"],
        "best_alpha_val_novel": alpha_summary["best_alpha_val_novel"],
        "alpha_scale_summary": alpha_summary["alpha_scale_summary"],
...
def main():
    ...
    parser.add_argument("--alpha_target_weight", type=float, default=0.1)
    parser.add_argument("--oracle_steps", type=int, default=20)
    parser.add_argument("--oracle_lr", type=float, default=0.05)
    parser.add_argument("--alpha_scales", type=float, nargs="+", default=[0.0, 0.25, 0.5, 0.75, 1.0])
    ...
    run_dataset(
        ...
        alpha_target_weight=args.alpha_target_weight,
