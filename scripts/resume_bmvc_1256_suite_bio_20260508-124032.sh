...
RUN_ID="${RUN_ID:-bmvc_1256_20260508-124032}"
...
ensure_seed_cfg() {
  local cfg_name="$1"
  local seed="$2"
  local src="configs/trainers/${cfg_name}"
  local dest="${CFG_DIR}/seed${seed}/${cfg_name}"
  if [[ ! -s "${dest}" ]]; then
    make_train_cfg_seed "${src}" "${dest}" "${seed}"
  fi
  printf '%s\n' "${dest}"
}
...
run_main_ablation() {
  ...
  cfg="$(ensure_seed_cfg "${cfg_name}" "${seed}")"
  run_command "${phase}" "${dataset}" "${trainer}" "${seed}" "train_cfg=${cfg_name}" \
...
log_master "RESUME_STARTED $(date '+%F %T %Z')"
...
for seed in "${SEED_LIST[@]}"; do
  for data_cfg in "${DATA_CFGS[@]}"; do
    run_main_ablation "ablation_bimc" "bimc.yaml" "bimc" "${seed}" "${data_cfg}"
    run_main_ablation "ablation_bimc_ensemble" "bimc_ensemble.yaml" "bimc_ensemble" "${seed}" "${data_cfg}"
    run_main_ablation "ablation_dino_fusion" "bimc_dino_fusion.yaml" "bimc_dino_fusion_ensemble" "${seed}" "${data_cfg}"
  done
done
