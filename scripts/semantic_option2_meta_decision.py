...
def latest_results(root, mode, train_cfg_stem):
    ...
    for dataset_dir in root.iterdir():
        if not dataset_dir.is_dir():
            continue
        trainer_dir = dataset_dir / f"semantic_meta_{mode}_{train_cfg_stem}"
...
def mode_decision(mode, payloads):
    gains = [float(v.get("method_gain", 0.0)) for v in payloads.values()]
    recoveries = [float(v.get("recovery_rate", 0.0) or 0.0) for v in payloads.values()]
    non_negative_all = all(gain >= 0.0 for gain in gains)

    if mode == "bmvc":
        strong_datasets = sum(gain >= 0.3 for gain in gains)
        continue_flag = non_negative_all and strong_datasets >= 2
    else:
        strong_datasets = sum(gain >= 0.5 for gain in gains)
        mean_recovery = sum(recoveries) / max(len(recoveries), 1)
        continue_flag = non_negative_all and strong_datasets >= 2 and mean_recovery >= 0.25

    return {
        "continue_flag": continue_flag,
        "non_negative_all": non_negative_all,
        "strong_datasets": strong_datasets,
        "mean_gain": round(sum(gains) / max(len(gains), 1), 4),
        "mean_recovery": round(sum(recoveries) / max(len(recoveries), 1), 4),
    }
